...
public class Main {
    public static void main(String[] args) throws IOException {
        ...
        // Render must be able to reach the server from outside the container.
    ...
    static class RootHandler implements HttpHandler {
        public void handle(HttpExchange ex) throws IOException {
            sendJson(ex, 200,
                    Json.message("status", "Railway Reservation API is running"));
        }
    }
    ...
    static class TrainsHandler implements HttpHandler {
        public void handle(HttpExchange ex) throws IOException {
            ...
            try {
                if (method.equals("GET")) listTrains(ex);
                else if (method.equals("POST")) addTrain(ex);
                else if (method.equals("PUT") && id != null) updateFare(ex, id);
                else if (method.equals("DELETE") && id != null) deleteTrain(ex, id);
                else sendError(ex, 404, "Route not found");
        ...
        void addTrain(HttpExchange ex) throws IOException, SQLException {
            ...
            String sql = "INSERT INTO TRAIN " +
                    "(train_name, source, destination, total_seats, available_seats, fare) " +
                    "VALUES (?,?,?,?,?,?)";
        ...
        void updateFare(HttpExchange ex, int id) throws IOException, SQLException {
            ...
            try (Connection con = DBConnection.getConnection();
                ...
                sendJson(ex, rows > 0 ? 200 : 404,
                        Json.message("status",
                                rows > 0 ? "Fare updated" : "Train not found"));
            }
        }
        ...
        void deleteTrain(HttpExchange ex, int id) throws IOException, SQLException {
            ...
            try (Connection con = DBConnection.getConnection();
                ...
                sendJson(ex, rows > 0 ? 200 : 404,
                        Json.message("status",
                                rows > 0 ? "Train deleted" : "Train not found"));
            }
        }
    }
    ...
    static class PassengersHandler implements HttpHandler {
        public void handle(HttpExchange ex) throws IOException {
            ...
            try {
                if (method.equals("GET")) listPassengers(ex);
                else if (method.equals("POST")) addPassenger(ex);
                else sendError(ex, 404, "Route not found");
            } catch (SQLException e) {
                sendError(ex, 500, "DB error: " + e.getMessage());
            }
        ...
        void addPassenger(HttpExchange ex) throws IOException, SQLException {
            ...
            String sql = "INSERT INTO PASSENGER " +
                    "(name, age, gender, phone, email) VALUES (?,?,?,?,?)";
    ...
    static class BookingsHandler implements HttpHandler {
        public void handle(HttpExchange ex) throws IOException {
            ...
            try {
                ...
                else if (method.equals("POST") && id == null) bookTicket(ex);
        ...
        void listBookings(HttpExchange ex) throws IOException, SQLException {
            ...
            String sql = "SELECT b.booking_id, p.name AS passenger, t.train_name, " +
                    "t.source, t.destination, b.seat_no, b.status, " +
        ...
        void cancelBooking(HttpExchange ex, int id) throws IOException, SQLException {
            ...
            try (Connection con = DBConnection.getConnection();
                ...
                sendJson(ex, rows > 0 ? 200 : 404,
                        Json.message("status",
                                rows > 0
                                        ? "Booking cancelled, seat released by trigger"
                                        : "No matching confirmed booking"));
            }
        }
    }
    ...
    static class NoBookingsHandler implements HttpHandler {
        public void handle(HttpExchange ex) throws IOException {
            ...
            List<String> rows = new ArrayList<>();
            String sql = "SELECT name, phone FROM PASSENGER " +
    ...
    static class ExpireWaitlistHandler implements HttpHandler {
        public void handle(HttpExchange ex) throws IOException {
            ...
            Map<String, String> p = queryParams(ex.getRequestURI());
            String before = p.get("before");

            if (before == null) {
                sendError(ex, 400, "Missing 'before' date param");
                return;
            }
            ...
            try (Connection con = DBConnection.getConnection();
                ...
                sendJson(ex, 200,
                        Json.message("status",
                                "Old waitlisted bookings before " + before + " expired"));
            } catch (SQLException e) {
                sendError(ex, 500, "DB error: " + e.getMessage());
            }
        }
